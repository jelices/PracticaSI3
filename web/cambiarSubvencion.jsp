...
function cargar()
{
	elementCS=document.getElementById('categoriaS');
	elementC=document.getElementById('categoria');
	for(var i=0; i<elementCS.options.length; i++)
		if(elementC.value==elementCS.options[i].value)
			elementCS.selectedIndex=i;
	elementAS=document.getElementById('ambitoS');
	elementA=document.getElementById('ambito');
	for(var i=0; i<elementAS.options.length; i++)
		if(elementA.value==elementAS.options[i].value)
			elementAS.selectedIndex=i;
	if(elementAS.selectedIndex>1)
	{
		modificar();
		elementLS=document.getElementById('lugarS');
		elementL=document.getElementById('lugar');
		for(var i=0; i<elementLS.options.length; i++)
			if(elementL.value==elementLS.options[i].value)
				elementLS.selectedIndex=i;
	}
}
...
<% 
	...
	Subvencion sub=null;
	if((session.getAttribute("usuario")!=null)&&(session.getAttribute("subvencionAEditar")!=null))
	{
		usuario=(Usuario)session.getAttribute("usuario");
		sub=(Subvencion)session.getAttribute("subvencionAEditar");
	}
...
<b>Modifique los datos deseados de la subvencion</b>
<form  method="POST" action="modificarSubvencion.do" >
<table>
<tr>
<td>Titulo: </td>
<td><input type="text" name="titulo" id="titulo" value="<%= sub.getTitulo() %>"></td>
</tr>
<tr>
<td>Categoria: </td>
<td>
...
<tr>
	...
	<td><textarea cols="40" rows="10" name="condiciones"><%= sub.getCondiciones()%></textarea></td>
</tr>
<tr>
...
<tr>
	...
	<td><textarea cols="40" rows="10" name="objeto"><%= sub.getObjeto() %></textarea></td>
</tr>
<tr>
	...
	<td><select id="lugarS" name="lugarS">
		...
		<input type="hidden" name="categoria" id="categoria" value="<%= sub.getCategoria() %>"/>
		<input type="hidden" name="ambito" id="ambito" value="<%= sub.getAmbito() %>"/>
		<input type="hidden" name="lugar" id="lugar" value="<%= sub.getLugar() %>"/>
<tr>
	<td></td>
	<td><input type="submit" onclick="copiar()" value="Modificar Subvencion"/>
	</td>
</tr>
</table>
</form>
